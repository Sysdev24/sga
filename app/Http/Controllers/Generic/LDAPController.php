        ...
        try {
            ...
            if ($connect) //Si todo ok, continua
            {
                ...
                if ($info['count'] === 0) {
                    ...
                    $arreglo_usuario['nombres'] = $usuario_mppee[0][0]["givenname"][0];
                    $arreglo_usuario['apellidos'] = $usuario_mppee[0][0]["sn"][0];
                } else {
                    $arreglo_usuario['email'] = $info[0]["mail"][0];
                    $arreglo_usuario['nombres'] = $info[0]["givenname"][0];
                    $arreglo_usuario['apellidos'] = $info[0]["sn"][0];
                }
                ...
                if ($info['count'] > 0) {
                    ...
                    for ($i = 0; $i < $info['count']; $i++) {
                        ...
                        array_push($arreglo_usuario,[
                            ...
                            'email'=> $info[$i]["mail"][0],
                            'nombres'=>strtoupper($info[$i]["givenname"][0]),
                            'apellidos'=>strtoupper($info[$i]["sn"][0]),
                            'photo'=> $foto
